...
bool MenuScene::init()
{
	...
	if (!init) {
		...
		Billboard* GL = new Billboard(menuShader, { Cappuccino::TextureLibrary::loadTexture("Logo billboard", "primordial-launcher-silhouette.png",Cappuccino::TextureType::PBRAlbedo) });
...
void MenuScene::childUpdate(float dt)
{
	...
	if (cursorBox.checkCollision(commandoBox, commandoBox._position, cursorBox._position) && characterSelect && charDelay <= 0.0f) {
		dynamic_cast<Cappuccino::UIText*>(ui._uiComponents[4])->setTextColour(glm::vec3(1.0f, 0.0f, 1.0f));
		...
		dynamic_cast<Cappuccino::UIText*>(ui._uiComponents[4])->setTextColour(glm::vec3(0.0f, 1.0f, 0.0f));

	//assault
	if (cursorBox.checkCollision(assaultBox, assaultBox._position, cursorBox._position) && characterSelect && charDelay <= 0.0f) {
		dynamic_cast<Cappuccino::UIText*>(ui._uiComponents[5])->setTextColour(glm::vec3(0.0f, 1.0f, 0.0f));
		if (_in.clickListener.leftClicked()) {
		...
		dynamic_cast<Cappuccino::UIText*>(ui._uiComponents[5])->setTextColour(glm::vec3(1.0f, 0.0f, 1.0f));

	//scout
	if (cursorBox.checkCollision(scoutBox, scoutBox._position, cursorBox._position) && characterSelect && charDelay <= 0.0f) {
		dynamic_cast<Cappuccino::UIText*>(ui._uiComponents[6])->setTextColour(glm::vec3(1.0f, 1.0f, 0.0f));
		if (_in.clickListener.leftClicked()) {
			Options::Scout = true;
			change = true;
		}
		...
		dynamic_cast<Cappuccino::UIText*>(ui._uiComponents[6])->setTextColour(glm::vec3(0.0f, 0.0f, 1.0f));

	//demolitionist
	if (cursorBox.checkCollision(demoBox, demoBox._position, cursorBox._position) && characterSelect && charDelay <= 0.0f) {
		dynamic_cast<Cappuccino::UIText*>(ui._uiComponents[7])->setTextColour(glm::vec3(0.0f, 1.0f, 1.0f));
		if (_in.clickListener.leftClicked()) {
		...
		dynamic_cast<Cappuccino::UIText*>(ui._uiComponents[7])->setTextColour(glm::vec3(1.0f, 0.0f, 0.0f));
